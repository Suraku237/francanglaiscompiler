...
async function exerciseTokenAnalysis(page: Page) {
  ...
  const wordRun = page.waitForResponse((response) => response.url().endsWith('/api/analyzer/analyze'))
  await page.getByRole('button', { name: 'Analyze', exact: true }).click()
  const word: AnalyzerResult = await (await wordRun).json()
  ...
  expect(result.corpus.lexical.total_tokens).toBe(5)
  await expect(page.getByText('REJECT', { exact: true })).toBeVisible()
  ...
  await expect(corpus.getByText('5 tokens · 5 distinct forms', { exact: true })).toBeVisible()
