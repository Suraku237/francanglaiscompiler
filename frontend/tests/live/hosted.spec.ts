...
test('real registration, shared compiler data, private sessions and reload persistence', async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(sharedTab.getByRole('heading', { name: 'Sign in to Camfranglais' })).toBeVisible()
...
test('real password recovery revokes existing sessions and replaces the old password', async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(recovery.getByRole('heading', { name: 'Sign in to Camfranglais' })).toBeVisible()
    await page.reload()
    await expect(page.getByRole('heading', { name: 'Sign in to Camfranglais' })).toBeVisible()
    await recovery.getByLabel('Email address').fill(email)
...
async function exerciseCompiler(page: Page) {
  ...
  await expect(page.getByRole('heading', { name: 'Vocabulary result', exact: true })).toBeVisible()
  expect(await page.getByLabel('Analyzed source text').textContent()).toBe('  123\t')
  ...
  await page.getByRole('link', { name: 'Back to Franc Analyzer' }).click()
  const conflictRun = page.waitForResponse((response) => response.url().endsWith('/api/analyzer/tests') && response.request().method() === 'POST')
  await page.getByRole('button', { name: 'Analyze', exact: true }).click()
  expect(await (await conflictRun).json()).toMatchObject({ approval: { accepted: true, unknown_count: 0 }, parse: { accepted: false } })
  await expect(page.getByText('ACCEPT', { exact: true })).toBeVisible()
  await page.getByRole('link', { name: 'View detailed analysis' }).click()
  await page.getByText('Saved grammar, transformations & FIRST/FOLLOW', { exact: true }).click()
  ...
  expect(allTests.summary).toMatchObject({ total: 3, accepted: 3, rejected: 0 })
  expect(allTests.grammar_summary).toMatchObject({ total: 3, accepted: 2, rejected: 1 })
...
async function exerciseTokenAnalysis(page: Page) {
  ...
  await expect(page.getByRole('region', { name: 'Vocabulary result' })).toBeVisible()
  await page.getByRole('link', { name: 'View detailed analysis' }).click()
  await expect(page.getByLabel('Analyzed source text')).toHaveText('Veux')
