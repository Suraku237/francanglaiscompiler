...
test('hash navigation, browser history and the skip link keep the active page and focus aligned', async ({ page }) => {
  ...
  await expect(navigation.getByRole('link', { name: 'Translator', exact: true })).toHaveAttribute('aria-current', 'page')
  await expect(page).toHaveTitle('Translator — Mboa language learning')
  ...
  await expect(page).toHaveTitle('Collection — Mboa language learning')
  await expect(page.getByRole('main')).toBeFocused()
  await expect(page.getByRole('button', { name: 'Add expression', exact: true })).toBeVisible()
...
test('native privacy dialog traps keyboard focus, closes with Escape and restores its trigger', async ({ page, isMobile }) => {
  ...
  const trigger = isMobile
    ? page.getByRole('button', { name: 'Privacy and voice information' })
    : page.getByRole('button', { name: 'Privacy & voice' })
  await trigger.click()
  const dialog = page.getByRole('dialog', { name: 'Your words. Your choice.' })
