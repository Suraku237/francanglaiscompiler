...
test('hash navigation, browser history and the skip link keep the active page and focus aligned', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Sentence to analyze' }).fill('Tab-only draft')
  ...
  await expect(page.getByRole('textbox', { name: 'Sentence to analyze' })).not.toBeVisible()

  await page.goBack()
  await expect(page).toHaveTitle('Compiler lab — Mboa Compiler')
  await expect(page.getByRole('textbox', { name: 'Sentence to analyze' })).toHaveValue('Tab-only draft')
  await expect(page.getByRole('main')).toBeFocused()
...
test('backend recovery updates compiler connectivity without sending or discarding a manual draft', async ({ page, api }) => {
  ...
  await page.getByRole('textbox', { name: 'Sentence to analyze' }).fill('Draft while offline')
  ...
  await expect(page.getByRole('textbox', { name: 'Sentence to analyze' })).toHaveValue('Draft while offline')
