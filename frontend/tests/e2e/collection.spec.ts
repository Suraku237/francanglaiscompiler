...
test('collection review validates text, invalidates approval and submits only changed fields', async ({ page, api }) => {
  ...
  const addDialog = page.getByRole('dialog', { name: 'Add terminology' })
  ...
  const editor = page.getByRole('dialog', { name: 'Review terminology' })
...
test('debounced search ignores stale results and client filters leave global counts intact', async ({ page, api }) => {
  ...
  await page.getByRole('searchbox', { name: 'Search terminology' }).fill('old')
  await expect.poll(() => api.calls('/api/dataset').filter((request) => request.url.searchParams.get('query') === 'old').length).toBe(1)
  await page.getByRole('searchbox', { name: 'Search terminology' }).fill('Second')
  ...
  await expect(page.getByRole('heading', { name: /Terminology library 1 entry/ })).toBeVisible()
