...
describe('all recorded test statistics', () => {
  it('shows exact totals, complete raw/normalized/category frequencies and unknown test counts', () => {
    ...
    expect(screen.getByRole('group', { name: 'Grammar matches' })).toHaveTextContent('1')
    expect(screen.getByRole('group', { name: 'Grammar mismatches' })).toHaveTextContent('2')
    expect(screen.getByRole('group', { name: 'Grammar match rate' })).toHaveTextContent('33.3%')
    ...
    expect(screen.getByText(/All completed public tests/)).toBeVisible()
    expect(screen.getAllByText('Public · Immutable saved test')).toHaveLength(3)
    expect(screen.queryByText(/Creator:|Second user/)).not.toBeInTheDocument()
