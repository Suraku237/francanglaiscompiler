...
function PrivacyDialog({ onClose }: { onClose: () => void }) {
  return <Modal title="Data & privacy" onClose={onClose} className="privacy-modal">
    <p className="modal-description">Your collection, recorded tests, grammar and recordings belong to your private account and selected project.</p>
    <div className="privacy-sections">
      <section><span className="privacy-section-icon"><Icon name="collection" size={21} /></span><div><h3>Collection and storage</h3><p>Analyze records each completed test, its text, grammar and results in your project, separately from Collection. Analysis statistics include repeated tests and remain after refresh or sign-out. A stopped request may still finish on the server; refresh saved tests to check.</p><p>Unsubmitted drafts stay in this tab and are lost on reload, project switch or sign-out. Save grammar edits explicitly. Approval records your review, not verified fieldwork.</p><p>Exports can include source context and contributor details. Share only with permission. Existing backups and legacy records remain on the server; removing their screens does not delete stored data. Contact the server operator for recovery.</p></div></section>
...
export default function App({ account, projects = [], selectedProject = 'default', onSelectProject, onSignOut }: {
  account?: Account
  projects?: Project[]
