"""Deterministic approved-example retrieval, never sentence synthesis or training."""
...
from .schemas import Coverage, DatasetLanguage, Evidence, TranslationContent, TranslationLanguage
...
def aligned_value(entry: dict[str, str], language: TranslationLanguage) -> str:
    if language == "fr":
        return entry.get("french_gloss", "")
    if language == "en":
        return entry.get("english_gloss", "")
    return entry.get("text", "") if entry.get("language") == language else ""
    ...
    *,
    ...
    for index, entry in enumerate(entries):
        if (
            ...
            or (local_languages and entry["language"] not in local_languages)
            or not entry.get("id") or not entry.get("text", "").strip()
        ):
            continue
        source = aligned_value(entry, source_language)
        target = aligned_value(entry, target_language)
        exact = bool(source.strip()) and normalize_text(source) == query_normalized[0]
        if exact:
        ...
        fields = [source]
        if chat:
            fields = [entry["text"], entry.get("french_gloss", ""), entry.get("english_gloss", "")]
        if any(len(entry.get(name, "")) > MAX_FIELD_CHARACTERS for name in ("text", "french_gloss", "english_gloss")):
            if exact:
                omitted = True
            continue
        score = (0, 0, 0)
    ...
    if result.ambiguous:
        result.coverage.warnings.append("Ambiguous approved alignment: full-entry matches have conflicting or missing target glosses.")
        ...
        if any(
            normalize_text(aligned_value(item.model_dump(), source_language)) == query_normalized[0]
            and aligned_value(item.model_dump(), target_language) == translation
            for item in result.evidence
        ):
            result.exact_translation = translation
    if result.exact_translation is None:
        result.coverage.warnings.append(
            "No unambiguous approved full-entry translation. Token/phrase evidence is not a complete sentence translation."
        )
    return result


def without_dataset(text: str) -> Grounding:
    return Grounding(coverage=Coverage(
        unmatched_terms=list(dict.fromkeys(terms(text))),
        warnings=["Dataset use is disabled; any translation is an unverified AI suggestion."],
    ))


def local_translation(result: Grounding, explanation_language: str) -> TranslationContent:
    if result.exact_translation is None:
        raise ValueError("No complete approved translation is available.")
