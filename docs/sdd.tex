...
and \texttt{audio\_utils.Recorder}, together with the relevant external
classes \texttt{customtkinter.CTk} and \texttt{sounddevice.InputStream}.
...
\texttt{audio\_utils.attach\_file(source, dir)} & Copies a source media file to the managed directory and returns its generated filename.\\
\texttt{audio\_utils.play\_audio(path)} & Returns true only when playback was successfully started.\\
